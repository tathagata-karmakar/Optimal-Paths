...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 24 20:34:09 2025
...
Initials = inits#jnp.array(inits)
G100 = np.matmul(Idmat[0], Initials)
G010 = np.matmul(Idmat[1], Initials)
k100 = np.matmul(Idmat[2], Initials)
k010 = np.matmul(Idmat[3], Initials)
G200 = np.matmul(Idmat[4], Initials)
G110 = np.matmul(Idmat[5], Initials)
G020 = np.matmul(Idmat[6], Initials)
k200 = np.matmul(Idmat[7], Initials)
k110 = np.matmul(Idmat[8], Initials)
k020 = np.matmul(Idmat[9], Initials)
...
cost_b = CostF_control_QT(Initials, X, P, H, X2, CXP, P2, rho_i, rho_f, l1max, ts, dt, tau, Idmat, Id)
...
while temp>tempf and (n<nsteps) and (-cost_b<.999):
  ...
  cost_n = CostF_control_QT(Initials_n, X, P, H, X2, CXP, P2, rho_i, rho_f, l1max, ts, dt, tau, Idmat, Id)
  if (cost_n<cost_b):
  ...
  diff = cost_n-cost_c
  metropolis = np.exp(-1000*diff/temp)
